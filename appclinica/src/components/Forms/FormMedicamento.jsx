...
export function FormMedicamento(){
    ...
    const userSchema = yup.object({
        ...
        prescripcion: yup.string()
        .required('La prescripción es requerida.')
        .min(10, 'La prescripción debe estar compuestos por más de 10 caracteres.')    })
    ...
    const { data, error, loaded } = useCallApi({ endpoint: `medicamento/${id}` })
            ...
            <Controller
                ...
                render={({ field })=>(
                  <TextField
                    ...
                    id='prescripcion'
                    label='Prescripción'
                    error={Boolean(errors.prescripcion)}
                    helperText={errors.prescripcion ? errors.prescripcion.message : ''}
                  />
                )}
               />     
            </FormControl>
