...
function TableToolbar(props){
  ...
  const info = () => {
    ...
    return navigate(`/informacion/${idSelected}`)
  }
  const cancelar = () => {
    console.log(idSelected)
    return navigate(`/cancelar/${idSelected}`)
  }
        ...
        ? (<>
           <Tooltip title='Más información...'>
            <IconButton onClick={info}>
              <PersonIcon key={idSelected} />
            </IconButton>
          </Tooltip>
          <Tooltip title='Cancelar Cita'>
            <IconButton onClick={cancelar}>
              <EventBusyIcon key={idSelected} />
            </IconButton>
          </Tooltip>
           </>)
        : (
...
export default function TableReserved(){
  ...
  const { data, error, loaded } = useCallApi({ endpoint: `cita/reserved/${routeParams.id}`})
